...
def PSOCP(T, n, c, s, Switches, Distances):
    personal_best_coor = np.zeros((n, 2*c))
    personal_best = [0.0 for i in range(0, n)]
    ...
    for i in range(0,n):
        personal_best_coor[i,:] = x[np.argmin(DELAY), :]
        personal_best[i] = DELAY[i]
    ...
    while t <= T:
        ...
        for i in range(0,n):
            for j in range(0,2*c):
                v[i,j] = 0.5*v[i,j] + 2*random.random()*(x[i,j]-personal_best_coor[i,j]) \
                         + 2*random.random()*(x[i,j]-x[global_best_id,j])
        x = x+v
...
if __name__ == '__main__':
    ...
    # PPP = np.zeros((5, c))
    DDD = np.ones((100, 1))
    for mmm in range(0,100):
        PSOCP(T,n,c,s,Switches,Distances)
    print(DDD.mean())
